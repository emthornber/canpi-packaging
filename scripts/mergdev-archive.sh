...
test -n "${get_keyring}" && (wget -O - https://repo.mergdev.org.uk/raspbian/gpg-pubkey.asc 2>/dev/null | gpg --dearmor - > /usr/share/keyrings/mergdev-archive-keyring.gpg)

echo "deb [signed-by=/usr/share/keyrings/mergdev-archive-keyring.gpg] https://repo.mergdev.org.uk/raspbian/ ${release} main" > /etc/apt/sources.list.d/mergdev.list
